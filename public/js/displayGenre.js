...
document.querySelector(".displayLabel").innerHTML = "Genre: " + window.location.pathname.substring(window.location.pathname.search("/genres/") + 8, window.location.pathname.length)
...
//Acquisition of all possible contents under searched keywords
for (let i = 0; i < genreSpecific.length; i++) {
    if (genreSpecific[i][0] == window.location.pathname.substring(window.location.pathname.search("/genres/") + 8, window.location.pathname.length)) {
        for (let j = 0; j < genreSpecific[i].length; j++) {
            if (j > 0) {
                obtainedResults.push(genreSpecific[i][j])
            }
        }
    }
}
